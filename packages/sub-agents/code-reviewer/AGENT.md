...
Reusable sub-agent that performs a **comprehensive review of a pull request's code diff**. Bundled into top-level agents that depend on it (no standalone build).
...
- `principles` — the review principles the reviewer must follow (string, or a list rendered as bullet points).
...
`review(input)` accepts the unified `diff` plus optional PR `title`, `description`, and `baseRef`, and returns the review as Markdown.
