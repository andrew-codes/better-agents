...
2. Extract the relevant reviewer feedback from that file. Post the substantive
   review content — the summary and findings. Do not invent feedback that is not in the file, and do not post empty or placeholder content.
3. Post it to the specified pull request using the provider's tools, as a single PR-level review comment, written as feedback from the user.
